...
public class GifCaptcha extends BaseCaptcha {
    ...
    private BufferedImage graphicsImage(final char[] chars, final Color[] fontColor, final char[] words, final int flag) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            //绘制椭圆边框
            g2d.drawOval(
                    RandomUtils.randomInt(width), RandomUtils.randomInt(height),
                    RandomUtils.randomInt(5, 30), 5 + RandomUtils.randomInt(5, 30)
            );
    ...
    private Color getRandomColor(int min, int max) {
        ...
        return new Color(RandomUtils.randomInt(min, max), RandomUtils.randomInt(min, max), RandomUtils.randomInt(min, max));
    }
}
