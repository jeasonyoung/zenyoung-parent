...
public class ShearCaptcha extends BaseCaptcha {
    ...
    public Image createImage(final String code) {
        ...
        // 画干扰线
        drawInterfere(g, 0, RandomUtils.randomInt(this.height) + 1, this.width,
                RandomUtils.randomInt(this.height) + 1, this.interfereCount, ImageUtils.randomColor());
        //
        return image;
    }
    ...
    private void shearX(final Graphics g, final int w1, final int h1, final Color color) {
        final int period = RandomUtils.randomInt(this.width);
        int frames = 1, phase = RandomUtils.randomInt(2);
        for (int i = 0; i < h1; i++) {
    ...
    private void shearY(final Graphics g, final int w1, final int h1, final Color color) {
        final int period = RandomUtils.randomInt(this.height >> 1);
