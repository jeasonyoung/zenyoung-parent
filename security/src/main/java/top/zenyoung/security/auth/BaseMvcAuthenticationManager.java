 ...
 */
@Slf4j
public abstract class BaseMvcAuthenticationManager<ReqBody extends LoginBodyDTO> extends BaseRestfulAuthenticationManager<ReqBody> {
    /**
     * 获取ObjectMapper对象
     *
    ...
    public void successfulAuthenticationHandler(@Nonnull final HttpServletResponse response, @Nonnull final UserPrincipal principal) {
        final LoginBodyVO respBody = buildSuccessfulLoginBody(principal);
     ...
     */
    @Nonnull
    protected abstract LoginBodyVO buildSuccessfulLoginBody(@Nonnull final UserPrincipal principal);
