...
public class BackgroundRemoval {
    ...
    public static BufferedImage backgroundRemoval(final BufferedImage bufferedImage, final Color override, int tolerance) {
        ...
        for (int y = image.getMinY(); y < image.getHeight(); y++) {
            for (int x = image.getMinX(); x < image.getWidth(); x++) {
                ...
                boolean isTrue = Lists.newArrayList(removeRgb).contains(hex) || areColorsWithinTolerance(hexToRgb(mainColor), new Color(Integer.parseInt(hex.substring(1), 16)), tolerance);
                if (isTrue) {
     ...
     */
    private static boolean fileTypeValidation(final File input, final String[] imagesType) throws IOException {
        if (!input.exists()) {
