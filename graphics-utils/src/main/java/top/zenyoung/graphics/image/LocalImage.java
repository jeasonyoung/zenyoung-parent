...
public class LocalImage implements Serializable {
     ...
     * @param srcImage 来源图片
     */
    public LocalImage(@Nonnull final BufferedImage srcImage) {
        this(srcImage, null);
    }
     ...
     */
    public LocalImage(@Nonnull final BufferedImage srcImage, @Nullable String targetImageType) {
        this.srcImage = srcImage;
        if (Strings.isNullOrEmpty(targetImageType)) {
            if (srcImage.getType() == BufferedImage.TYPE_INT_ARGB || srcImage.getType() == BufferedImage.TYPE_INT_ARGB_PRE
                    || srcImage.getType() == BufferedImage.TYPE_4BYTE_ABGR || srcImage.getType() == BufferedImage.TYPE_4BYTE_ABGR_PRE
            ) {
     ...
     */
    public LocalImage setTargetImageType(@Nonnull final String imgType) {
     ...
     * @return 处理后的图像
     */
    public LocalImage pressText(@Nonnull final String pressText, @Nonnull final Color color, @Nullable final Font font,
     ...
     * @return 处理后的图像
     */
    public LocalImage pressText(@Nonnull final String pressText, @Nonnull final Color color, @Nullable Font font,
                                @Nonnull final Point point, final float alpha) {
     ...
     * @return 长宽信息
     */
    public static Dimension getDimension(@Nonnull final FontMetrics metrics, @Nonnull final String str) {
     ...
     * @return 处理后的图像
     */
    public LocalImage pressTextFull(@Nonnull final String pressText, @Nonnull final Color color, @Nullable Font font,
        ...
        final int targetHeight = targetImage.getHeight(), targetWidth = targetImage.getWidth();
        // 创建画笔，并设置透明度和角度
        final Graphics2D g = targetImage.createGraphics();
        g.setColor(color);
        // 基于图片中心旋转
     ...
     * @return this
     */
    public LocalImage pressImage(@Nonnull final Image pressImg, final int x, final int y, final float alpha) {
        final int pressImgWidth = pressImg.getWidth(null), pressImgHeight = pressImg.getHeight(null);
     ...
     * @return this
     */
    public LocalImage pressImage(@Nonnull final Image pressImg, @Nonnull final Rectangle rectangle, final float alpha) {
     ...
     * @return this
     */
    public LocalImage stroke(@Nullable final Color color, final float width) {
     ...
     * @return this
     */
    public LocalImage stroke(@Nullable final Color color, @Nullable final Stroke stroke) {
     ...
     */
    public boolean write(@Nonnull final OutputStream out) throws IOException {
     ...
     * @throws IOException IO异常
     */
    public boolean write(@Nonnull final ImageOutputStream targetImageStream) throws IOException {
     ...
     * @return 绘制后的背景
     */
    private BufferedImage draw(@Nonnull final BufferedImage backgroundImg, @Nonnull final Image img,
                               @Nonnull final Rectangle rectangle, final float alpha) {
     ...
     * @param alpha 透明度：alpha 必须是范围 [0.0, 1.0] 之内（包含边界值）的一个浮点数字
     */
    private static void setAlpha(@Nonnull final Graphics2D g, final float alpha) {
        g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
    }
     ...
     */
    private void fixRectangle(@Nonnull final Rectangle rectangle, final int baseWidth, final int baseHeight) {
        if (this.positionBaseCentre) {
